# Changes for macOS:
# .type integer2string, @function removed

# Changes for 64-Bit:
# eXX => rXX
# pushl, popl,  movl, addl, incl, decl, cmpl, divl =>  pushq, popq,  movq, addq, incq, decq, cmpq, divq
# Stack size doubled


# PURPOSE: Convert an interger number to a decimal string
#          for display.
#
# INPUT:   A buffer large enough to hold the largest possible
#	   number.
#	   An integer to convert.
#
# OUTPUT:  The buffer will be overwritten with the decimal
#	   string
#
# Variables:
#
...
# When we divide by 10, the 10 must be in a register
# or memory location.
movq $10, %rdi

conversion_loop:
 # Division is acutally performed on the combined 
 ...
 # Quotient is in the right place. %rdx has the
 # reminder, which now needs to be converted into
 # a number. So, %rdx has a number that is 0 through 9.
 # You could also interpret this as an index on the
 # ASCII table, starting from the character '0'. The
 # ASCII code for '0' plus zero is still the ASCII code
 # for '0'. The ASCII code for '0' plus 1 is the ASCII
 # code for the character '1'. Therefore, the following
 ...
 # but we only need the byte in %dl (the last byte of 
 # the %rdx register) for the character.
 pushq %rdx
 
 # Increment the digit count
 incq %rcx

 # Check to see if %rax is zero yet, got to next step
 #  if so
 cmpq $0, %rax
...
copy_reversing_loop:
 # We push a whole register, but we only need the 
 # last byte. So we are going to pop off to the entire
 # %rax register, but then only move the small part (%al)

...
conversion_loop:
 ...
 # a number. So, %edx has a number that is 0 through 9.
 ...
 # the %edx register) for the character.
 pushl %edx
 
 # Increment the digit count
 incl %ecx

 # Check to see if %eax is zero yet, got to next step
 #  if so
 cmpl $0, %eax
...
copy_reversing_loop:
 ...
 # %eax register, but then only move the small part (%al)
